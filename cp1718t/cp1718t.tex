...
Construindo um anamorfismo que aplica \emph{(const 1.0)} ou \emph{(split (((sqrt(2)/2)^) . succ) (split id id)))} dependendo do resultado de aplicar \emph{outNat}, conseguimos construir a respetiva PTree. 
...
\\[\blanklineskip]%
\>[B]{}\Varid{dist}\mathrel{=}\Varid{aux}\comp \conj{\Varid{id}}{\Varid{sum}\comp \map \;(\p2)}\comp \Varid{unB}{}\<[E]%
=======
\>[B]{}\Varid{singletonbag}\;\Varid{x}\mathrel{=}\mathit B\;[\mskip1.5mu (\Varid{x},\mathrm{1})\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\mu \mathrel{=}\bot \mbox{\onelinecomment concat . map p1 }{}\<[E]%
>>>>>>> invert
\\
\>[B]{}\hsindent{7}{}\<[7]%
